...
Multi Hub 是一個輕量級的桌面應用程式，將常用的網路服務整合在同一個視窗中，無需開啟瀏覽器。透過左側邊欄與右上角 Floating Dock 切換的方式，讓你可以快速存取 Messenger、ChatGPT、Gemini AI、Git Updater、Git GUI、Skill Sync、Quick Notes、Discord 與 Telegram。此外，還內建了強大的 **AI 大管家 (AI Butler)**，隨時協助處理各項任務。

---

## 主要功能

- **AI 大管家 (AI Butler)** - 內建浮動式 AI 助理（支援 OpenAI/Gemini），具備上下文感知能力，可操作 Quick Notes、Skill Sync、Git GUI 等工具，支援多會話歷史管理。
...
- **Quick Notes** - 內建筆記功能，支援 Markdown / Mermaid 語法、Slash Command (`/`) 快速輸入、語法檢查與互動式預覽。
- **Markdown Helper** - 內建 Markdown 語法秘笈與常用指令選單，提升筆記效率。
- **Git Repo Updater** - 批次更新本地 Git 儲存庫，顯示分支 / dirty / behind 狀態
- **Git GUI** - 本地 Git 圖形介面：commit、branch、stash、diff、status、push/pull 等
- **Skill Sync** - 跨專案 `.cursor/skills` 技能檔案比較與同步（支援多資料夾選取、捲動式目錄樹結構）。
- **系統匣常駐** - 關閉視窗後最小化至系統匣
- **快捷鍵設定** - Alt+1~7 自訂綁定服務（支援多服務輪播），強化 Master-Detail 管理介面與拖曳排序。
- **視窗記憶** - 自動記住視窗位置與大小
- **單一實例** - 防止重複開啟
...
| `Alt+A` | 呼叫/隱藏 AI 大管家 (AI Butler) 面板 |
...
| `F11` | 全螢幕 |
| `Esc` | 關閉 Popup / Modal |
| `/` | (Quick Notes) 開啟 Slash Command 選單 |
| `Ctrl+I` | (Quick Notes) 開啟 Markdown 語法秘笈 |

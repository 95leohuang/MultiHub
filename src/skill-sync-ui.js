document.addEventListener('DOMContentLoaded', async () => {
    ...
    scanBtn.onclick = async () => {
        ...
        if (!rootPath) return alert('Please enter root path');
    ...
    syncSelectedBtn.onclick = async () => {
        ...
        if (!confirm(`Synchronize all versions of ${previewContext.filename} using [${previewContext.leftRepo}] as the master source?`)) return;
